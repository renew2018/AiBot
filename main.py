...
@app.post("/upload_standard_pdf", summary="Upload a PDF to the Standards folder")
async def upload_standard_pdf(
    ...
    sanitized_name = "".join(c for c in standard_name if c.isalnum() or c in (' ', '_', '-')).rstrip()
    save_path = os.path.join(STANDARDS_DIR, f"{sanitized_name}.pdf")

    with open(save_path, "wb") as f:
        f.write(await file.read())

    return {"message": f"Uploaded '{sanitized_name}.pdf' to Standards folder."}
...
@app.delete("/delete_standard", summary="Delete a PDF from Standards folder by name")

async def delete_standard(standard_name: str = Form(...)):
    STANDARDS_DIR = "Standards"
    # Apply same sanitization as during upload
    sanitized_name = "".join(c for c in standard_name if c.isalnum() or c in (' ', '_', '-')).rstrip()
    file_path = os.path.join(STANDARDS_DIR, f"{sanitized_name}.pdf")

    if not os.path.exists(file_path):
        raise HTTPException(status_code=404, detail=f"Standard '{sanitized_name}.pdf' not found.")

    try:
        os.remove(file_path)
        return {"message": f"Deleted standard '{sanitized_name}.pdf' from Standards folder."}
    except Exception as e:
